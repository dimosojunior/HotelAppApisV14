...
class RetailsFoodProductsViewSet(APIView):
...
class RetailsDrinksProductsViewSet(APIView):
    def get(self, request):
        try:
            # Get the page number from the query parameters, default to 1
            page = int(request.query_params.get('page', 1))
...
class RetailsOthersProductsViewSet(APIView):
    def get(self, request):
        try:
            # Get the page number from the query parameters, default to 1
            page = int(request.query_params.get('page', 1))
            ...
            queryset = RetailsOthersProducts.objects.filter(
                productCategory__id__icontains = categoryId
                )
            ...
            serializer = RetailsOthersProductsSerializer(page_items, many=True)
...
class RetailsOthersCategoriesViewSet(APIView):
    def get(self, request):
        try:
            # Get the page number from the query parameters, default to 1
            page = int(request.query_params.get('page', 1))
            ...
            serializer = RetailsOthersCategoriesSerializer(page_items, many=True)
